...
@section('content')
    ...
    @php
        ...
        $raidos = \App\Raid::whereRaw('now() < DATE_ADD(end_time, INTERVAL 2 HOUR) ORDER BY raid_tier DESC')->count();
    @endphp
    @if($raidos == 0)-
            ...
            <div id="all-raids" class="tab-pane fade in active show">
                @php
                  //  $timeDiff = (env('UTC_TIME_DIFFERENCE') * -1);
                    $raids = \App\Raid::whereRaw('now() < DATE_ADD(end_time, INTERVAL 2 HOUR) ORDER BY raid_tier DESC')->get();
                @endphp
